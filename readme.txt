...
archive pages if the `no_add_to_cart` option is set to 'hide'.

Display a button that is just the product permalink if
the `no_add_to_cart` option is set to 'permalink'.

Set the button label globally by setting the `no_add_to_cart_permalink_text`
option, set the button label per product type by setting
the `no_add_to_cart_permalink_{product_type}_text` option.
